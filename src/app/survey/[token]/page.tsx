 ...
 *
 * Audit fixes: rotated sad-face mascot removed (calmer tone for someone who
 * just cancelled); checked pill was off-palette blue-500 and translated 2px
 * on select (grid jiggled) — now teal-700 fill with no movement; focus rings
 * blue → teal; submit button no longer uppercase-shouting.
 *
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-4 md:px-6 py-12">
      <div className="w-full max-w-2xl">
        {isPreview && (
          <div className="mb-6 rounded-3xl border-2 border-teal-400 bg-teal-400/15 dark:bg-teal-400/10 px-5 py-3 text-center text-sm font-bold text-teal-800 dark:text-teal-300">
        ...
        <div className="bg-white dark:bg-[#121214] rounded-[2.5rem] shadow-2xl shadow-zinc-200/50 dark:shadow-black/50 border border-zinc-100 dark:border-zinc-800 p-8 md:p-14 transition-colors duration-500">
          {/* Header — no mascot; calm and direct */}
          ...
          <form action="/api/survey" method="POST" className="space-y-10">
            ...
            {/* Submit — quieter than a marketing CTA */}
            <div className="flex justify-center">
              <button
                type="submit"
                className="px-10 py-3.5 rounded-full bg-teal-900 dark:bg-teal-100 font-bold text-sm text-white dark:text-teal-950 hover:bg-teal-800 dark:hover:bg-white transition-colors"
              >
                Send feedback
              </button>
            </div>
