...
// on file is whatever /api/auth/request stored at signup.
...
function OnboardingFormInner() {
  ...
  const planSuffix = plan === 'starter' || plan === 'growth' ? `&plan=${plan}` : '';
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          // The server component already checks this on page load, but the
          // session can lapse in the gap between that render and this submit
          // (cookie expiry, logout in another tab). The route's own 401 is the
          // real source of truth, not just the page-load check.
          router.push(`/login?next=/onboarding${planSuffix}`);
          return;
        }
